...
/* Representation of a point in time or of the duration of a time interval,
 * as an unsigned 32-bit value in milliseconds. */
typedef NUAN_U32 NUAN_TIME;

typedef struct NUAN_CLOCK_DATA_S
{
  /* Representation of a time value as a triple containing
   * (real, user, system) time values:
   * real time:   the wall time;
   * user time:   amount of time spent by the current thread in user-mode;
   * system time: amount of time spent by the current thread in kernel-mode. */
  NUAN_TIME rtime;
  NUAN_TIME utime;
  NUAN_TIME stime;
} NUAN_CLOCK_DATA;
...
struct VE_CLOCK_INTERFACE_S {
  /* Return by reference the clock data representing the elapsed time
   * since the clock time origin, which should be stored in the hClock. */
  NUAN_ERROR (*pfGetRelativeTime)(void            * hClock,
                                  NUAN_CLOCK_DATA * pClockData);
};

typedef void* (*VPLATFORM_THREAD_STARTFUNC)(void*);

struct VE_MTHREAD_INTERFACE_S {
NUAN_ERROR (*pfOpen)(void* phClass, void* pHeap, void ** pThreadH);
NUAN_ERROR (*pfClose)(void * pThreadH);
NUAN_ERROR (*pfStart)(void * pThreadH, 
                             VPLATFORM_THREAD_STARTFUNC pStartFunction,
                             void* pArgs,
                             size_t pStackSize);
NUAN_ERROR (*pfJoin)(void * pThreadH,void* pStatus);
NUAN_ERROR (*pfSleepMs)(void *pThreadH ,unsigned short pMS);
NUAN_ERROR (*pfCallingId)(void *pThreadH, unsigned int *pThreadId);
};

struct VE_SEMAPHORE_INTERFACE_S {
NUAN_ERROR (*pfOpen )(void* pHclass, void* pHeap,
                                   signed int pInit,signed int pMax,void ** pSem);
NUAN_ERROR (*pfClose)(void * pSem);
NUAN_ERROR (*pfAcquire )(void * pSem);
NUAN_ERROR (*pfRelease )(void * pSem);
};
