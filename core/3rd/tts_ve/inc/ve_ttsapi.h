...
/* --------------------------- COPYRIGHT INFORMATION -----------------------+
 ...
 | Cerence, Inc. and also is protected as an unpublished work               |
 | under applicable Copyright laws. Recipient is to retain this program in  |
 | confidence and is not permitted to use or make copies thereof other than |
 | as permitted in a prior written agreement with Cerence, Inc. or its      |
 | affiliates.                                                              |
 | Copyright (c) 2019 Cerence, Inc.                                         |
...
struct VE_CLOCK_INTERFACE_S;
typedef struct VE_CLOCK_INTERFACE_S VE_CLOCK_INTERFACE;

struct VE_MTHREAD_INTERFACE_S;
typedef struct VE_MTHREAD_INTERFACE_S VE_MTHREAD_INTERFACE;

struct VE_SEMAPHORE_INTERFACE_S;
typedef struct VE_SEMAPHORE_INTERFACE_S VE_SEMAPHORE_INTERFACE;
...
typedef struct {
  ...
  ** log handle during ve_ttsOpen( ) */
  void                                 * hLog;
  /* Clock interface, used to retrieve OS-agnostic clock values,
  ** may be NULL to disable clock-dependent functionalities */
  const VE_CLOCK_INTERFACE             * pIClock;
  /* Clock handle for timing operations */
  void                                 * hClock;
  /* Thread interface, used to retrieve OS-agnostic threading primitives:
  *  - thread management
  *  - semaphores
  *  - events
  ** may be NULL to disable thread-dependent functionalities */
  const VE_MTHREAD_INTERFACE            * pIThread;
  const VE_SEMAPHORE_INTERFACE          * pISemaphore;
...
typedef enum {
  ...
  VE_PARAM_PITCH                  = 10,  /* parameter to set desired pitch of request audio or to know value of default or already set pitch */
  ...
  VE_PARAM_VOP_VERSION            = 21,  /* Version of voice package aka VOP, this parameter is used to set desired VOP version or to know VOP version */
  VE_PARAM_DISABLE_FINAL_SILENCE  = 22,
    /*  Voice configuration  */
  VE_PARAM_NOCLMLANGUAGES         = 23,   /* This is the list of foreign languages that a VOP can read (using foreign sounds).
                                            The value is a string with a comma-separate list of 3-letter language codes e.g "enu,frc,spm". The value can only be queried by ve_ttsGetParamList().
                                            It can not be modified by ve_ttsSetParamList() */
  VE_PARAM_TIMBRE                 = 24
} VE_PARAMID;
...
typedef  NUAN_ERROR  VE_TTSGETCLMINFO_F (
  const VE_HSPEECH          hTtsCl,
  const char              * szLanguage,
        VE_CLMINFO        * pClmInfo
);
...
typedef  NUAN_ERROR  VE_TTSGETADDITIONALPRODUCTINFO_F (
  VE_ADDITIONAL_PRODUCTINFO  * pTtsAdditionalProductInfo
);

typedef  const char *  VE_TTSGETERRORSTRING_F (
  NUAN_ERROR  errorCode
);
...
VEAPI_EXTERN const char * VEAPI_EXPORT ve_ttsGetErrorString (
  NUAN_ERROR  errorCode
);
