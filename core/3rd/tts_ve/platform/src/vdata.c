...
/*------------------------------------------------------------------*/
NUAN_ERROR vplatform_data_GetStreamClassData(
    void *  hDataClass,
    void **pStreamClassData)
{
  VPLATFORM_DATA_CLASS_T * pClass = (VPLATFORM_DATA_CLASS_T *)hDataClass;

  *pStreamClassData = pClass->pStreamClassData;

  return NUAN_OK;
} /* vplatform_data_GetPlatformInfo */

/*------------------------------------------------------------------*/
NUAN_ERROR vplatform_data_SetStreamClassData(
    void *  hDataClass,
    void *pStreamClassData )
{
  VPLATFORM_DATA_CLASS_T * pClass = (VPLATFORM_DATA_CLASS_T *)hDataClass;

  pClass->pStreamClassData  = pStreamClassData;

  return NUAN_OK;
} /* vplatform_data_SetStreamClassData */
    ...
    VPLATFORM_RESOURCES * pResources)
{
    ...
    if (pResources->u16NbrOfDataInstall != 0)
    {
        ...
        /* Keep a copy of ppFileList moved to the end of the list,
         * so that in case of an error, only the additional found files
         * will be removed again, and not the original list in the data class
         */
        VPLATFORM_FILELIST ** ppFileListCopy = ppFileList;
        while (*ppFileListCopy != NULL)
        {
            ppFileListCopy = &((*ppFileListCopy)->pNext);
        }
        /* When there is no check on duplicates,
         * there's no need to start from the beginning of the list
         */
        if (pClass->bErrorCheck == NUAN_FALSE)
        {
            ppFileList = ppFileListCopy;
        }
        ...
        if (fRet != NUAN_OK)
        {
             vplatform_FreeFiles(pClass->hHeap, NULL, *ppFileListCopy);
             *ppFileListCopy = NULL;
        }
    }
    return fRet;
...
#ifdef VSTREAM_CLASS_DATA
  /* initialize vstream class data */
  fRet = vplatform_datastream_Init((void * )pClass, hHeap);
  if (fRet != NUAN_OK)
  {
      vplatform_heap_Free(hHeap, pClass);
      return fRet;
  }
#endif /* VSTREAM_CLASS_DATA  */
...
#endif
#ifdef VSTREAM_CLASS_DATA
  /* deinitialize vstream class data */
  vplatform_datastream_Uninit((void * )pClass);
#endif /* VSTREAM_CLASS_DATA  */
